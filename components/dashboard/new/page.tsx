  ...
  const handleSubmitForReview = async () => {
    ...
    try {
      ...
      if (!createResponse.ok) {
        ...
        console.error("[v0] Create article failed:", errorData)
        alert(`Failed to create article: ${errorData.error || "Unknown error"}`)
        return
      }

      const article = await createResponse.json()
      console.error("[v0] Article created successfully:", article)

      if (!article.id) {
        console.error("[v0] No article ID returned:", article)
        alert("Failed to get article ID")
        return
      }
      ...
      if (submitResponse.ok) {
        ...
        console.error("[v0] Submit failed:", submitError)
        alert(`Failed to submit article for review: ${submitError.error || "Unknown error"}`)
      }
    } catch (error) {
